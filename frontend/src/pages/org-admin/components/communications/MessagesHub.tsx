  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <div>
          <p className="text-sm font-semibold text-neutral-900">Tema visivo</p>
          <p className="mt-1 text-xs text-neutral-500">Scegli il tono complessivo della mail.</p>
        </div>
        <EmailLayoutPicker
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-[1.4rem] border border-neutral-200 bg-neutral-50 p-4">
          <p className="text-sm font-semibold text-neutral-900">Preset font</p>
          <div className="mt-3 grid gap-2">
            {fontPresets.map((preset) => (
              <button
                key={preset.key}
                type="button"
                className={`${baseButtonClass} ${draft.design.font_preset === preset.key ? "border-brand bg-brand/5 text-brand" : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-300 hover:bg-neutral-50"}`}
                onClick={() => patchDesign({ font_preset: preset.key })}
              >
                <p className="text-sm font-semibold">{preset.title}</p>
                <p className="mt-1 text-xs text-neutral-500">{preset.sample}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-[1.4rem] border border-neutral-200 bg-neutral-50 p-4">
          <p className="text-sm font-semibold text-neutral-900">Stile pulsante</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {ctaStylePresets.map((preset) => (
              <button
                key={preset.key}
                type="button"
                className={`${baseButtonClass} ${draft.design.cta_style === preset.key ? "border-brand bg-brand/5 text-brand" : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-300 hover:bg-neutral-50"}`}
      ...
      <div className="rounded-[1.4rem] border border-neutral-200 bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-neutral-900">Hero image</p>
          ...
          <div className="inline-flex rounded-full border border-neutral-200 bg-neutral-50 p-1">
            {([
              ...
              <button
                key={option.key}
                type="button"
                className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${draft.header_mode === option.key ? "bg-neutral-900 text-white" : "text-neutral-600"}`}
                onClick={() => {
        ...
        {draft.header_mode === "form" && !linkedForm?.cover_image_url ? <div className="mt-4 rounded-[1rem] border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Il modulo selezionato non ha un'immagine disponibile.</div> : null}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-[1.4rem] border border-neutral-200 bg-white p-5 space-y-4">
          <div>
          ...
          <div className="grid gap-2">
            {([{ key: "association", label: "Usa logo associazione" }, { key: "upload", label: "Carica nuovo logo" }, { key: "none", label: "Nessun logo" }] as Array<{ key: LogoMode; label: string }>).map((option) => (
              <button
                key={option.key}
                type="button"
                className={`${baseButtonClass} ${draft.logo_mode === option.key ? "border-brand bg-brand/5 text-brand" : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-300 hover:bg-neutral-50"}`}
                onClick={() => {
          ...
          <label className="flex items-center gap-3 rounded-[1.2rem] border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-700"><input type="checkbox" className="rounded border-neutral-300 text-brand" checked={draft.design.show_association_name} onChange={(event) => patchDesign({ show_association_name: event.target.checked })} /><span>Mostra nome associazione</span></label>
        </div>

        <div className="rounded-[1.4rem] border border-neutral-200 bg-white p-5 space-y-4">
          <p className="text-sm font-semibold text-neutral-900">Palette</p>
          <div className="grid gap-4 md:grid-cols-2">
            <label className={labelClass}>
              Colore principale
      ...
      <div className="rounded-[1.4rem] border border-neutral-200 bg-white p-5 space-y-4">
        <div>
          <p className="text-sm font-semibold text-neutral-900">Blocchi opzionali</p>
          <p className="mt-1 text-xs text-neutral-500">Aggiungi solo ciò che serve al messaggio.</p>
        </div>
        <div className="grid gap-4 xl:grid-cols-2">
          <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-neutral-900">Immagine nel contenuto</p><button type="button" className="text-xs font-semibold text-brand" onClick={() => patchDesign({ secondary_image_url: draft.design.secondary_image_url ? "" : draft.design.hero_image_url || "" })}>{draft.design.secondary_image_url ? "Rimuovi" : "Aggiungi"}</button></div>
            {draft.design.secondary_image_url ? <ImageUpload label="Immagine contenuto" value={draft.design.secondary_image_url || null} onChange={(base64) => patchDesign({ secondary_image_url: base64 })} onRemove={() => patchDesign({ secondary_image_url: "" })} /> : <p className="text-xs text-neutral-500">Puoi inserire un'immagine secondaria dentro il messaggio.</p>}
          </div>
          <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-neutral-900">Box evidenza</p><button type="button" className="text-xs font-semibold text-brand" onClick={() => patchDesign({ highlight_title: "", highlight_body: "" })}>Svuota</button></div>
          ...
          <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4 space-y-3 xl:col-span-2">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-neutral-900">Dettagli evento</p><button type="button" className="text-xs font-semibold text-brand" onClick={() => patchDesign({ event_details: "" })}>Svuota</button></div>
          ...
          <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-neutral-900">Firma</p><button type="button" className="text-xs font-semibold text-brand" onClick={() => patchDesign({ signature_name: "", signature_role: "" })}>Svuota</button></div>
          ...
          <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-neutral-900">Nota finale</p><button type="button" className="text-xs font-semibold text-brand" onClick={() => patchDesign({ final_note: "" })}>Svuota</button></div>
      ...
      <details className="rounded-[1.4rem] border border-neutral-200 bg-neutral-50 px-5 py-4">
        ...
        <div className="mt-4 grid gap-4">
          ...
          <div className="rounded-[1rem] border border-dashed border-neutral-300 bg-white px-4 py-3 text-sm text-neutral-500">
            TODO backend: libreria asset condivisa e upload email dedicato ancora da completare.
          </div>
        </div>
      </details>
    </div>
  ...
  return (
    <div className="rounded-[1.6rem] border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-neutral-700">{getLayoutLabel(template.design.layout_key)}</span>
      </div>
      <div className={`mt-4 rounded-[1.2rem] border border-neutral-200 p-4 ${normalizeDesign(template.design).layout_key === "reminder" ? "bg-blue-50" : normalizeDesign(template.design).layout_key === "event" ? "bg-orange-50" : normalizeDesign(template.design).layout_key === "institutional" ? "bg-slate-50" : normalizeDesign(template.design).layout_key === "elegant" ? "bg-amber-50" : "bg-neutral-50"}`}>
...
function CampaignWizardStepper(props: { currentStep: number }) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-4">
      <div className="rounded-[1.6rem] border border-neutral-200 bg-white px-4 py-4 shadow-sm md:hidden">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-neutral-500">Step {currentStep + 1} di {wizardSteps.length}</p>
            <p className="mt-1 text-lg font-semibold text-neutral-950">{activeStep.title}</p>
          </div>
          <span className="rounded-full border border-brand/20 bg-brand/5 px-3 py-1 text-xs font-semibold text-brand">{Math.round(progress)}%</span>
        </div>
            ...
            {wizardSteps.map((step, index) => {
              ...
              return (
                <div key={step.key} ref={(node) => { mobileStepRefs.current[index] = node; }} className="flex w-[4.6rem] shrink-0 flex-col items-center text-center">
                  <span
                    className={`relative z-10 flex h-11 w-11 items-center justify-center rounded-full border text-sm font-semibold transition ${
                      completed || active
                        ? "border-brand bg-gradient-to-br from-brand to-sky-500 text-white shadow-[0_12px_28px_rgba(15,118,110,0.2)]"
      ...
      <div className="relative hidden px-3 pt-2 md:block">
        <div
          className="absolute left-[7%] right-[7%] top-8 h-[3px] rounded-full bg-neutral-200"
          aria-hidden="true"
        />
        <div
          ...
          {wizardSteps.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center text-center">
                <span
                  className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-full border text-sm font-semibold transition ${
                    completed || active
                      ? "border-brand bg-gradient-to-br from-brand to-sky-500 text-white shadow-[0_14px_32px_rgba(15,118,110,0.22)]"
                      : "border-neutral-200 bg-white text-neutral-400"
                  }`}
                >
                  {completed ? "OK" : index + 1}
                ...
                <span className={`mt-1 text-sm font-medium leading-5 ${completed || active ? "text-neutral-800" : "text-neutral-400"}`}>
                  {step.title}
                </span>
              </div>
            );
...
export function MessagesHub({
          ...
          <div className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
            <section className="space-y-5">
              <div className={panelClass}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand">Campagna</p>
                    <h2 className="mt-2 text-2xl font-bold tracking-tight text-neutral-900">{selectedCampaign.name || selectedCampaign.subject}</h2>
                    <p className="mt-2 text-sm text-neutral-500">{selectedCampaign.subject}</p>
                  </div>
                  <span className="rounded-full border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-neutral-700">{statusLabel(selectedCampaign.status)}</span>
                ...
                <div className="mt-5 grid gap-4 md:grid-cols-2">
                  <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs text-neutral-500">Destinatari</p><p className="mt-2 text-xl font-semibold text-neutral-900">{selectedCampaign.recipient_count || selectedCampaign.planned_recipient_count}</p><p className="mt-1 text-sm text-neutral-500">{selectedCampaign.target_summary}</p></div>
                  <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs text-neutral-500">Tema</p><p className="mt-2 text-xl font-semibold text-neutral-900">{getLayoutLabel(selectedCampaign.design.layout_key)}</p><p className="mt-1 text-sm text-neutral-500">{getCtaLabel(selectedCampaign.design.cta_kind)}</p></div>
                  <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs text-neutral-500">Modulo collegato</p><p className="mt-2 text-xl font-semibold text-neutral-900">{selectedCampaign.linked_form?.title || "Nessuno"}</p><p className="mt-1 text-sm text-neutral-500">{selectedCampaign.linked_form ? "La CTA apre un modulo pubblico nel browser." : "Nessun modulo collegato."}</p></div>
                  <div className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs text-neutral-500">Invio</p><p className="mt-2 text-xl font-semibold text-neutral-900">{formatDateTime(selectedCampaign.sent_at || selectedCampaign.scheduled_at || selectedCampaign.created_at)}</p><p className="mt-1 text-sm text-neutral-500">Inviate {selectedCampaign.recipient_status_counts.sent} · Fallite {selectedCampaign.recipient_status_counts.failed}</p></div>
                </div>
                <div className="mt-5 flex flex-wrap gap-2">
                  {["draft", "scheduled"].includes(String(selectedCampaign.status || "").toLowerCase()) ? <button type="button" className="btn-primary !text-sm" disabled={communicationsLocked} onClick={() => void sendOrgAdminEmailCampaign(selectedCampaign.id).then(() => refreshCampaigns(selectedCampaign.id)).catch((err) => handleLoadError(err, "Errore durante l'invio campagna."))}>Invia ora</button> : null}
                  <button type="button" className="btn-secondary !text-sm" onClick={() => openCampaignComposerFromCampaign(selectedCampaign)}>Apri come nuova bozza</button>
    ...
    return (
      <div className="space-y-8">
        <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-neutral-500">{editingExistingCampaign ? "Modifica campagna" : "Nuova campagna"}</p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-neutral-950">{currentStep.title}</h2>
            <p className="mt-2 text-sm text-neutral-500">Un solo passaggio alla volta, con preview forte solo quando serve davvero.</p>
          </div>
          <button
            type="button"
        ...
        {!compactComposerPreview && !fullComposerPreview ? (
          <div className="mx-auto w-full max-w-4xl">
            <div className="rounded-[2.1rem] border border-neutral-200 bg-white px-6 py-7 shadow-[0_28px_80px_-46px_rgba(15,23,42,0.16)] md:px-10 md:py-10">
              <div className="mx-auto max-w-3xl space-y-8">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand">{currentStep.eyebrow}</p>
                  <h3 className="mt-3 text-2xl font-semibold tracking-tight text-neutral-950">{currentStep.title}</h3>
                </div>
                ...
                <div className="rounded-[1.6rem] border border-neutral-200 bg-neutral-50 px-5 py-4">
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-neutral-500">Riepilogo rapido</p>
                  <p className="mt-2 text-sm text-neutral-700">Tipo: <span className="font-semibold text-neutral-900">{communicationTypes.find((item) => item.value === campaignDraft.communication_type)?.title || "Comunicazione"}</span></p>
                  <p className="mt-1 text-sm text-neutral-700">Destinatari stimati: <span className="font-semibold text-neutral-900">{estimate ?? "-"}</span></p>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-neutral-100 pt-6">
                  ...
                  <div className="text-sm text-neutral-500">Step {wizardStep + 1} di {wizardSteps.length}</div>
        ...
        {compactComposerPreview ? (
          <div className="mx-auto grid w-full max-w-6xl gap-6 xl:grid-cols-[0.92fr_1.08fr]">
            <section>
              <div className="rounded-[2.1rem] border border-neutral-200 bg-white px-6 py-7 shadow-[0_28px_80px_-46px_rgba(15,23,42,0.16)] md:px-8 md:py-8">
          ...
          <div className="mx-auto w-full max-w-6xl space-y-6">
            <div className="rounded-[2.1rem] border border-neutral-200 bg-white px-6 py-7 shadow-[0_28px_80px_-46px_rgba(15,23,42,0.16)] md:px-8 md:py-8">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand">{currentStep.eyebrow}</p>
                  <h3 className="mt-3 text-2xl font-semibold tracking-tight text-neutral-950">Controllo finale della mail</h3>
                </div>
            ...
            <div className="rounded-[2.1rem] border border-neutral-200 bg-white p-5 shadow-[0_28px_80px_-46px_rgba(15,23,42,0.16)] md:p-7">
              <EmailPreview loading={campaignPreviewLoading} preview={campaignPreview} view={previewView} onViewChange={setPreviewView} />
            </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-neutral-500">Campagne</p>
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-neutral-900">Elenco campagne</h2>
        </div>
        <button type="button" className="btn-primary" onClick={openNewCampaignComposer}>Nuova campagna</button>
      </div>
      ...
      <div className={`${panelClass} p-0`}>
        {campaignListItems.length === 0 ? (
          <div className="p-10 text-center text-sm text-neutral-500">Nessuna campagna trovata. Crea la prima bozza guidata.</div>
        ) : campaignListItems.map((campaign) => (
          <div key={campaign.id} className="border-b border-neutral-100 px-5 py-4 last:border-b-0">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <button
                type="button"
              ...
              >
                <div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-neutral-700">{statusLabel(campaign.status)}</span><span className="text-xs text-neutral-500">{getLayoutLabel(campaign.design.layout_key)}</span></div>
                <p className="mt-3 text-sm font-semibold text-neutral-900">{campaign.name || campaign.subject}</p>
