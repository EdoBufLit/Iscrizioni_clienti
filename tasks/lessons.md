...
- Se il requisito richiede particle logo ‚Äúpuro‚Äù, non mantenere mai un underlay immagine quando WebGL e attivo: il fallback statico va mostrato solo quando WebGL e disabilitato/non disponibile.
...
- Backend che richiede config (es. statute_pdf_path) per TUTTE le entit√† blocca quelle senza config. Rendere i check condizionali: se l'org non ha statuto caricato, non richiederlo.
...
- In un flusso multi-step (join + register), se step 1 √® critico e step 2 √® opzionale, gestirli separatamente: marcare successo dopo step 1 e rendere step 2 non-bloccante.
...
- Quando una route di provisioning puÔøΩ colpire DB legacy, non fare commit() cieco: intercettare IntegrityError e gestire fallback/409 per evitare 500 lato UI.
...
- Quando si rimuove una variabile da un componente React/TS (es. eliminando una branch condizionale), verificare sempre che non resti una dichiarazione `const x = ...` senza uso: TypeScript noUnusedLocals bloccher√† il build in CI/Docker.
...
- Se i numeri tessera devono essere riutilizzabili dopo delete, non basta pulire member.card_no: va anche riavvolto card_batches.next_no (con lock) al numero rilasciato, altrimenti líallocazione continua dal progressivo successivo.
...
- L'etichetta Google Wallet "[SOLO TEST]" non Ë generata dal codice app: resta finchÈ issuer/class non sono approvati in produzione da Google Wallet.
...
- Se il cliente quantifica il fix layout (es. "20% pi˘ largo"), implementare quel rapporto in modo esplicito nel CSS/classi e non continuare con tentativi incrementali in `rem`.
...
- Se un modulo usa un provider diverso per un sottoinsieme di email (es. association mode), il test-email admin non deve limitarsi ad accodare: deve chiamare davvero quel transport e restituire subito líerrore provider/config reale al frontend.
...
- Se una feature ha gia tutte le funzioni richieste ma continua a essere percepita come "editor tecnico", trattare link pubblico, stato e workflow summary come elementi di primo livello nel layout: la gerarchia visiva Ë parte del prodotto, non rifinitura opzionale.
...
- Nei sample payment/demo che convivono con flussi Stripe reali gi‡ presenti, nomi route, etichette UI e documentazione devono contenere esplicitamente `demo` per impedire ambiguit‡ future con il prodotto reale.
...
- Se una migration nuova tocca tabelle che il repo puÚ gi‡ creare via `create_all()` o bootstrap, il downgrade non deve dropparle alla cieca: va reso conservativo e non distruttivo, lasciando la tabella intatta quando la provenance non Ë certa.
- Quando sostituisco un editor esistente con un builder UI, devo rimuovere subito stati/handler legacy non pi˘ usati e tipizzare esplicitamente helper condivisi (`props`, payload field, callback) prima di chiudere: altrimenti `npm run build` si rompe per dead code, import drift o ritorni `unknown`.
...
- In pratica: gli org sempre a `0` o comunque sempre sotto soglia non devono ricevere alert iniziali solo perchÈ il valore corrente e basso.
